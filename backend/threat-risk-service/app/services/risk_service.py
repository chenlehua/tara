...
from tara_shared.constants.tara import RISK_LEVEL_TO_CAL, RISK_MATRIX
...
class RiskService:
    ...
    def assess_risk(
        ...
        threat = (
            self.db.query(ThreatRisk).filter(ThreatRisk.id == threat_risk_id).first()
        )
    ...
    def get_risk_summary(self, project_id: int) -> Dict[str, Any]:
        ...
        total = (
            self.db.query(func.count(ThreatRisk.id))
            .filter(ThreatRisk.project_id == project_id)
            .scalar()
        )
